import { MagnifyingGlassIcon } from "@heroicons/react/24/outline";
import { CheckIcon, EyeIcon, XMarkIcon } from "@heroicons/react/24/solid";
import {
  Button,
  CardBody,
  CardFooter,
  CardHeader,
  Chip,
  IconButton,
  Input,
  Rating,
  Spinner,
  Tooltip,
  Typography,
} from "@material-tailwind/react";
import { useEffect, useState } from "react";
import { Link } from "react-router-dom";
import {
  getSearchedUsers,
  getUsers,
  updateSellerStatus,
} from "../../../services/admin_service";
import { ADMIN_SELLERS_REVIEWS_PATH } from "../../constants";
import SellerApproveModal from "./SellerApproveModal";
import TrashSellerModal from "./TrashSellerModal";

const AdminSellersTable = () => {
  const [open, setOpen] = useState(false);
  const [approveOpen, setApproveOpen] = useState(false);
  const [sellers, setSellers] = useState([]);
  const [isLoading, setIsLoading] = useState(false);
  const [selectedId, setSelectedId] = useState(null);
  const [selectedStatus, setSelectedStatus] = useState(null);

  const handleOpen = () => setOpen(!open);
  const handleApproveOpen = () => setApproveOpen(!approveOpen);
  const TABLE_HEAD = ["Sellers", "Ratings", "Status", "Created date", ""];

  const TABLE_ROWS = [
    {
      img: "/products/fish -3.jpg",
      name: "John Michael",
      email: "john@creative-tim.com",
      job: "48",
      online: true,
      date: "23/04/18",
    },
    {
      img: "/products/fish -3.jpg",
      name: "Alexa Liras",
      email: "alexa@creative-tim.com",
      job: "77",
      online: false,
      date: "23/04/18",
    },
    {
      img: "/products/fish- 1.jpg",
      name: "Laurent Perrier",
      email: "laurent@creative-tim.com",
      job: "33",
      online: false,
      date: "19/09/17",
    },
    {
      img: "/products/fish- 2.jpg",
      name: "Michael Levi",
      email: "michael@creative-tim.com",
      job: "71",
      online: true,
      date: "24/12/08",
    },
    {
      img: "/products/fish -3.jpg",
      name: "Richard Gran",
      email: "richard@creative-tim.com",
      job: "71",
      online: false,
      date: "04/10/21",
    },
  ];

  const getSellersData = () => {
    setIsLoading(true);
    getUsers()
      .then((res) => {
        if (res.data.result) {
          setSellers(res.data.users);
          setIsLoading(false);
        }
      })
      .catch(() => setIsLoading(false));
  };

  useEffect(() => {
    getSellersData();
  }, []);

  const searchSeller = (value) => {
    if (value != "") {
      const obj = {
        userName: value,
      };

      getSearchedUsers(obj).then((res) => {
        if (res.data.result) {
          setSellers(res.data.users);
        }
      });
    } else {
      getUsers().then((res) => {
        if (res.data.result) {
          setSellers(res.data.users);
        }
      });
    }
  };

  const updateStatus = (id, status) => {
    const obj = {
      id,
      status,
    };

    updateSellerStatus(obj).then((res) => {
      if (res.data.result) {
        getSellersData();
      }
    });
  };

  return (
    <div className="h-full w-full">
      <CardHeader floated={false} shadow={false} className="rounded-none">
        <div className="mb-8 flex items-center justify-between gap-8">
          <div>
            <Typography variant="h5" color="blue-gray">
              Sellers
            </Typography>
            <Typography color="gray" className="mt-1 font-normal">
              See information about all sellers
            </Typography>
          </div>
        </div>
        <div className="flex flex-col items-center justify-between gap-4 md:flex-row">
          <div className="w-full md:w-72">
            <Input
              label="Search"
              onChange={(e) => searchSeller(e.target.value)}
              icon={<MagnifyingGlassIcon className="h-5 w-5" />}
            />
          </div>
        </div>
      </CardHeader>
      {isLoading ? (
        <div className="w-full h-40 flex justify-center items-center">
          <Spinner color="blue" />
        </div>
      ) : (
        <div className="border border-gray-400 rounded-lg mt-10 overflow-hidden">
          <CardBody className="overflow-scroll px-0">
            <table className="w-full min-w-max table-auto text-left -mt-6">
              <thead>
                <tr>
                  {TABLE_HEAD.map((head) => (
                    <th
                      key={head}
                      className="border-y border-blue-gray-100 bg-blue-gray-50/50 p-4"
                    >
                      <Typography
                        variant="small"
                        color="blue-gray"
                        className="font-normal leading-none opacity-70"
                      >
                        {head}
                      </Typography>
                    </th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {sellers.map(
                  (
                    { _id, firstName, lastName, email, status, createdAt },
                    index
                  ) => {
                    const isLast = index === TABLE_ROWS.length - 1;
                    const classes = isLast
                      ? "p-4"
                      : "p-4 border-b border-blue-gray-50";

                    return (
                      <tr key={index}>
                        <td className={classes}>
                          <div className="flex items-center gap-3">
                            <div className="flex flex-col">
                              <Typography
                                variant="small"
                                color="blue-gray"
                                className="font-normal"
                              >
                                {firstName} {lastName}
                              </Typography>
                              <Typography
                                variant="small"
                                color="blue-gray"
                                className="font-normal opacity-70"
                              >
                                {email}
                              </Typography>
                            </div>
                          </div>
                        </td>
                        <td className={classes}>
                          <div className="flex flex-col">
                            <Rating value={4} readonly />
                          </div>
                        </td>
                        <td className={classes}>
                          <div className="w-max">
                            <Chip
                              variant="ghost"
                              size="sm"
                              value={
                                status === 4
                                  ? "Active"
                                  : status === 0
                                  ? "Pending"
                                  : "Declined"
                              }
                              color={
                                status === 4
                                  ? "green"
                                  : status === 0
                                  ? "blue-gray"
                                  : "red"
                              }
                            />
                          </div>
                        </td>
                        <td className={classes}>
                          <Typography
                            variant="small"
                            color="blue-gray"
                            className="font-normal"
                          >
                            {createdAt}
                          </Typography>
                        </td>
                        <td className={`${classes} flex space-x-5`}>
                          <Tooltip content="View Reviews">
                            <Link to={ADMIN_SELLERS_REVIEWS_PATH}>
                              <IconButton variant="text">
                                <EyeIcon className="h-4 w-4" />
                              </IconButton>
                            </Link>
                          </Tooltip>
                          {status === 0 && (
                            <Tooltip content="Approve Seller">
                              <IconButton
                                variant="text"
                                onClick={() => {
                                  setSelectedId(_id);
                                  setSelectedStatus(4);
                                  setApproveOpen(true);
                                }}
                              >
                                <CheckIcon className="h-4 w-4" />
                              </IconButton>
                            </Tooltip>
                          )}

                          {status === 0 && (
                            <Tooltip content="Reject Seller">
                              <IconButton
                                variant="text"
                                onClick={() => {
                                  setSelectedId(_id);
                                  setSelectedStatus(404);
                                  setOpen(true);
                                }}
                              >
                                <XMarkIcon className="h-4 w-4" />
                              </IconButton>
                            </Tooltip>
                          )}
                        </td>
                      </tr>
                    );
                  }
                )}
              </tbody>
            </table>
          </CardBody>
        </div>
      )}

      <CardFooter className="flex items-center justify-between border-t border-blue-gray-50 p-4">
        <Typography variant="small" color="blue-gray" className="font-normal">
          Page 1 of 10
        </Typography>
        <div className="flex gap-2">
          <Button variant="outlined" size="sm">
            Previous
          </Button>
          <Button variant="outlined" size="sm">
            Next
          </Button>
        </div>
      </CardFooter>
      <TrashSellerModal
        handleStatus={() => updateStatus(selectedId, selectedStatus)}
        handleOpen={handleOpen}
        open={open}
      />
      <SellerApproveModal
        handleStatus={() => updateStatus(selectedId, selectedStatus)}
        handleOpen={handleApproveOpen}
        open={approveOpen}
      />
    </div>
  );
};

export default AdminSellersTable;
